...
function Header(){
   ...
   //$this->Cell(16,0.5, $this->Image('logo.jpg', $this->GetX(), $this->GetY(),1.7,1.7,0,0), 0, 0, 'L', false );
    ...
    //$this->Ln(0);
   $this->Cell(31,1,'REKAPITULASI NILAI PENGETAHUAN',0,0,'C');
   $this->Ln(1.5);
   //$this->Ln(0.5);

   //$this->Cell(30,0,$mapel['nama_mapel'],0,0,'C');
   //$this->Ln(0.5);
   //$this->Cell(30,0,'SEMESTER '.$smt.' TAHUN PELAJARAN '.$tapel,0,0,'C');
   //$this->Ln(1);
   ...
   $this->Ln(1.2);
   $this->SetTextColor(0,0,0);
   $ceklebar=$connect->query("select * from pemetaan where kelas='$ab' and smt='$smt' and kd_aspek='3' and mapel='$mp'")->num_rows;
   $lebar=23/($ceklebar*3);
   ...
   while($s=$peta->fetch_assoc()){
	   $tema=$s['tema'];
	   $ckd=$connect->query("select * from pemetaan where kelas='$ab' and smt='$smt' and kd_aspek='3' and mapel='$mp' and tema='$tema'")->num_rows;
	   $kd=$connect->query("select * from pemetaan where kelas='$ab' and smt='$smt' and kd_aspek='3' and mapel='$mp' and tema='$tema'")->fetch_assoc();
	   $lebars1=3*$lebar*$ckd;
   ...
   while($s1=$peta1->fetch_assoc()){
	   $tema1=$s1['tema'];
	   $ckd1=$connect->query("select * from pemetaan where kelas='$ab' and smt='$smt' and kd_aspek='3' and mapel='$mp' and tema='$tema1'")->num_rows;
	   $lebars2=3*$lebar;
	   $que2="select * from pemetaan where kelas='$ab' and smt='$smt' and kd_aspek='3' and mapel='$mp' and tema='$tema1' order by nama_peta asc";
   ...
   $this->Ln(0.8);
   $this->SetX($x);
   $que2="select * from pemetaan where kelas='$ab' and smt='$smt' and kd_aspek='3' and mapel='$mp'";
   $peta2=$connect->query($que2);
   while($s2=$peta2->fetch_assoc()){
	   $this->Cell($lebar,0.8,'U','LTBR',0,'C',1);
	   $this->Cell($lebar,0.8,'T1','LTBR',0,'C',1);
	   $this->Cell($lebar,0.8,'T2','LTBR',0,'C',1);
   }
   $this->Ln(0.8);
...
$qsis = $connect->query("select * from penempatan where rombel='$kelas' and tapel='$tapel' and smt='$smt' order by nama asc");
   ...
   $pdf->SetTitle("Rekapitulasi Nilai Pengetahuan ~ APINS");
   $pdf->SetAuthor("APINS");
   ...
   while($siswa=$qsis->fetch_assoc()){
	   ...
	   $que2="select * from pemetaan where kelas='$ab' and smt='$smt' and kd_aspek='3' and mapel='$mp' order by tema, nama_peta asc";
	   $peta2=$connect->query($que2);
	   
	   while($s2=$peta2->fetch_assoc()){
		   $tema=$s2['tema'];
		   $kd=$s2['nama_peta'];
		   $m1=$connect->query("select * from nh where id_pd='$idp' and smt='$smt' and tapel='$tapel' and mapel='$mp' and tema='$tema' and kd='$kd' and jns='tls'")->fetch_assoc();
		   $m2=$connect->query("select * from nh where id_pd='$idp' and smt='$smt' and tapel='$tapel' and mapel='$mp' and tema='$tema' and kd='$kd' and jns='tgs1'")->fetch_assoc();
		   $m3=$connect->query("select * from nh where id_pd='$idp' and smt='$smt' and tapel='$tapel' and mapel='$mp' and tema='$tema' and kd='$kd' and jns='lsn'")->fetch_assoc();
		   if($m1['nilai']==0){
...
$pdf->Cell(10,0.5,'Keterangan',0,0,'L');
   ...
   $pdf->Ln(0.5);
   $pdf->Cell(4,0.3,'U = Test Tertulis',0,0,'L');
$pdf->Ln(0.5);
$pdf->Cell(4,0.3,'T1 = Tugas 1',0,0,'L');
$pdf->Ln(0.5);
$pdf->Cell(10,0.3,'T2 = Tugas 2',0,0,'L');
