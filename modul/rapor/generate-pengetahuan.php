	...
	$adar=mysqli_num_rows(mysqli_query($koneksi, "select * from raport_k13 where id_pd='$idp' AND kelas='$ab' AND smt='$smt' AND tapel='$tapel' AND mapel='$mpid' and jns='k3'"));
		...
		while($ze=mysqli_fetch_array($sqkd)){
			...
			if($chin>0){
				...
				if($nak>0)
					mysqli_query($koneksi, "insert into nh_temp(id_pd,kelas,smt,tapel,mapel,kd,nph) values('$idp','$ab','$smt','$tapel','$mpid','$kd1','$nak')");
			};
		}; //kd
		if($ra2>0){
		...
		if($ada>0){
			...
			if($ads>0){
				...
				mysqli_query($koneksi, "INSERT INTO raport(id_pd,kelas,smt,tapel,mapel,nilai,oleh) VALUES('$idp','$ab','$smt','$tapel','$mpid','$nakhir','')");
			};
		}else{
			mysqli_query($koneksi, "INSERT INTO raport_k13(id_pd,kelas,smt,tapel,mapel,jns,nilai,predikat,deskripsi) VALUES('$idp','$ab','$smt','$tapel','$mpid','k3','$nakhir','$predikat','$desk')");
			if($ads>0){
				$srapors=mysqli_fetch_array(mysqli_query($koneksi, "select * from raport where id_pd='$idp' AND kelas='$ab' AND smt='$smt' AND tapel='$tapel' AND mapel='$mpid'"));
				$idns=$srapors['id_raport'];
				mysqli_query($koneksi, "UPDATE raport SET nilai='$nakhir' WHERE id_raport='$idns'");
...
$validator['messages'] = "Rapor Pengetahuan atas nama ".$nm['nama']." berhasil di Generate!";
