...
$output = array('data' => array());
	...
	while ($row = $query->fetch_assoc()) {
		...
		$cekr = $connect->query("select * from data_register where peserta_didik_id='$idp'")->num_rows;
		$regs = $connect->query("select * from data_register where peserta_didik_id='$idp'")->fetch_assoc();
		$nisn=$row['nisn'];
		$jk=$row['jk'];
		$ids=$row['id'];
		$rmb=$pn['rombel'];
		...
		if($status!='1'){
			if($cekr>0){
				...
				<a href="'.base_url().'edit-alumni/'.$idp.'" class="btn btn-outline-primary btn-sm me-1 mb-1">
				...
				$actionButton = '
				<button class="btn btn-outline-danger btn-icon btn-sm me-1 mb-1">
