...
if(is_numeric($nilai)){
    if($nilai>100){}else{
		...
		$vck = $connect->query("select * from temp_pas where id_pd='$pdid' AND kelas='$kelas' AND smt='$smt' AND tapel='$tapel' AND mapel='$mapel'")->num_rows;
        $vrh=$connect->query("select avg(nilai) as rerata from nats where id_pd='$pdid' AND kelas='$ab' AND smt='$smt' AND tapel='$tapel' AND mapel='$mapel'")->fetch_assoc();
        $rerata=$vrh['rerata'];
        if($vck>0){
        	$vcn=$connect->query("select * from temp_pas where id_pd='$pdid' AND kelas='$kelas' AND smt='$smt' AND tapel='$tapel' AND mapel='$mapel'")->fetch_assoc();
        	$idt=$vcn['idNH'];
